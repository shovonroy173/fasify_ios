...
const ProviderCarDashboardScreen = () => {
  ...
  const { data: allStatics, isLoading, isError } = useGetAllStaticsCarQuery();
  ...
  console.log("LINE AT 107", allCars, carsData);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ActiveListingCarCard
            key={item.id}
            item={item}
            name={"selectedActiveBusinessCar"}
        ...
        ListEmptyComponent={
          <View>
            ...
            {/* Inline empty state */}
            {!carsLoading && !carsError && !carsData?.data?.length && (
              <GlobalNoData />
            )}
          </View>
        }
...
export default ProviderCarDashboardScreen;
