...
const ProviderHotelDashboardAllRoomScreen = ({ route }) => {
  ...
  // console.log('LINE AT 44', hotelsData, route?.params?.id, hotelsError);
    ...
    >
      <View className="flex-row justify-between items-center">
        <View className="flex-row gap-8 items-center">
          ...
          <ThemedText styles="text-lg font-SemiBold">All Rooms</ThemedText>
        </View>
        <Pressable
          onPress={() =>
            navigation.navigate('Dashboard', {
              screen: 'ProviderAddHotelRoomListing',
            })
          }
      ...
      <FlatList
        data={allHotels}
        renderItem={({ item }) => (
          <ActiveListingRoomCard
            key={item.id}
            item={item}
            name={'selectedActiveListingRoom'}
            navigation={navigation}
            path="Dashboard"
            screen="ProviderEditHotelRoomListing"
          />
        )}
        ...
        ListEmptyComponent={
          hotelsLoading ? (
            ...
            <View style={{ alignItems: 'center', marginTop: 40 }}>
              <ThemedText>No rooms found.</ThemedText>
            </View>
          )
        }
...
export default ProviderHotelDashboardAllRoomScreen;
